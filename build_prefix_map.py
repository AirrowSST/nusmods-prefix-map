...
function build(mods){
  ...
  for(const m of mods){
    ...
    // 1-4 undergraduate; 5-7 graduate; 8-9 are administrative placeholders
    // (Advance Placement Credit, Credit Transfer, Exchange) and no-digit codes are
    // not taught modules, so they count toward neither level. Default lean is UG.
    if('1234'.includes(dig))e.ug++;
    else if('567'.includes(dig))e.grad++;
