...
function apply(){
  ...
  document.querySelectorAll('.fac').forEach(fac=>{
    ...
    fac.querySelectorAll('.dept').forEach(dep=>{
      let depVis=false,hiddenN=0;
      dep.querySelectorAll('.chip').forEach(ch=>{
        const okS=facMatch||ch.dataset.search.includes(t);
        const okL=!lo||(+ch.dataset.offered>0);
        const okM=!hm||(+ch.dataset.total>mn);
        const okG=!hg||ch.dataset.gradonly!=='1';
        const show=okS&&okL&&okM&&okG;
        ch.classList.toggle('hide',!show); if(show)depVis=true;
        if(!(okL&&okM&&okG))hiddenN++;
      });
      dep.querySelector('.dept-hidden').textContent=hiddenN?hiddenN+' hidden':'';
...
function updateStats(){
  ...
  document.querySelectorAll('.chip:not(.hide)').forEach(ch=>{
